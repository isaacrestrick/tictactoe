...
export const MatchList = (props) => {
  ...
  return (
    <div className="flex flex-col items-center">
        ...
        {gamesVisible && games.map(game => <li key={game.id}><Game id={game.id}/></li>)}
        {!gamesVisible && games.map(game => <div key={game.id} className="bg-white border black mt-4 ml-4 w-[30%] h-10 flex flex-col items-center justify-center" onClick={() => setMatchId(game.id)}><p className="ml-2 text-md">game: {game.id}</p></div>)}
    </div>
  )
